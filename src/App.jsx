...
export default function App() {
  ...
  const [invoiceCount, setInvoiceCount] = useState(
    () => parseInt(localStorage.getItem("billkaro_count") || "0", 10)
  );
  const [isPro, setIsPro] = useState(
    () => localStorage.getItem("billkaro_pro") === "true"
  );
  ...
  const doDownload = () => {
    if (!isPro) {
      ...
      localStorage.setItem("billkaro_count", String(newCount));
    }
    window.print();
  };
  ...
  const openRazorpay = () => {
    ...
    const options = {
      ...
      currency: "INR",
      name: "BillKaro",
      description: "Pro Plan - Unlimited Invoices",
      handler: function (response) {
        localStorage.setItem("billkaro_pro", "true");
        setIsPro(true);
        doDownload();
      },
      theme: { color: "#d4af37" },
  ...
  // ── Print (kept for internal use; paywall wraps it via handlePaywall) ────────
  const handlePrint = () => handlePaywall();
  ...
  return (
    <div>
      ...
      <div className="print-actions" style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        {/* Dynamic Download / Upgrade button */}
        {isPro ? (
          <button
          ...
          <button
            ...
            onClick={handlePaywall}
          >
            ⬇ Download PDF ({freeLeft} free left)
          </button>
        ) : (
          <button
            style={{
